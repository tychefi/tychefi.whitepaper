...
## 行业背景
当今全球经济由于种种原因进入一个大的下行周期，整体经济因此产生通缩和消费降级。大部分用户不仅缺乏金融知识和专业的投资理财技能，也十分缺乏优良投资理财标的物来获得中长期的可靠收益。许多用户出于侥幸心理而追求暴利暴富，却大概率遭受严重损失。全球无数用户对未来经济发展和财富积累产生迷茫并失去了方向。
...
2. 天池财富的矿池按照投入锁仓时间划分不同矿池，实现动态年化收益（APY: `2～20%`），其中锁仓时间越长和投入资金量越大的收益率相对越高；
...
`天池财富`的总矿池(`tyche.earn`)按照投入锁仓时间区分为以下五个矿池：
- `天池#1`: 锁仓1天
- `天池#2`: 锁仓30天
- `天池#3`: 锁仓90天
- `天池#4`: 锁仓180天
...
- 天池`#5`矿池内每100USDT投入即可挖得`1`枚`TYCHE`，其中`0.8`枚同步锁仓在池内，`0.2`枚进入用户钱包。
...
| 风险储备金：取自整体收益部分，不参与任何投资理财。 | 零风险 | 零收益 | 无 | `10%` |
...
| 分配对象              | 分配占比    | 分配数量    | 分配使用目的                                             | 锁仓释放计划            |
| -------------------- | ---------- | ---------- | ------------------------------------------------------ | --------------------- |
| 市场建设              | `2%`       | `42万`      | 社区空投（天池矿池中）、开拓市场、社区发展、用户支持            | 按贡献释放，无锁仓要求 |
| 创始团队              | `15%`      | `315万`     | 开发用户产品、理财投资业务、用户支持 | 产品研发和建设预留`105万`，后面`210万`在市值每增长`$10万USDT`，则解锁1万枚`TYCHE`给团队 |
| 战略合作              | `8%`       | `168万`     | 理财业务合作、空投合作、挖矿合作                            | 按贡献释放，无锁仓要求 |
| 天池#5质押挖矿         | `50%`      | `1050万`    | 用户直接质押挖矿，收益+TYCHE双挖 | 按比例分配的天赐币，其中`90%`同质押币一起释放，`10%`直接释放 |
| 基金会 (`tyche.fund`) | `25%`      | `525万`     | 全部严格用于市值管理，稳定市场，推动生态建设健康发展为唯一目标   | 由`TycheFi`的DAO组织来统一管理 |
| 投资量化研究院         | `5%`       | `105万`     | 加密领域技术和产品投资标的物研究，量化策略和系统研发           | 预留`15万`用于早期建设，后面按贡献释放，无锁仓要求 |
...
- 平台承诺每年至少拿出净利润部分的`10%`用来回购销毁`$TYCHE`，确保其价值稳步上升，直到最后总量通缩到`100万枚`；
